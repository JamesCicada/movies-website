...
export async function getvidsrc(
  tmdb_id: string,
  s?: string,
  e?: string
  ...
  try {
    ...
    const firstDecode = (await string_atob(mEncrypt))
      .split(".")
      .map((item) => item.split("").reverse().join(""));
    const secondDecode = JSON.parse(
      await string_atob(firstDecode.join("").split("").reverse().join(""))
    );
    ...
    for (let i = 0; i < secondDecode.length; i++) {
      ...
      directQuality.sort((a: any, b: any) => b.quality - a.quality);
...
async function string_atob(input: string): Promise<string> {
  const chars =
  ...
  for (
    let bc = 0, bs = 0, buffer = undefined, i = 0;
    (buffer = str.charAt(i++));
